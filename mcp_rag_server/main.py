...
QDRANT_COLLECTION = os.getenv("QDRANT_COLLECTION", "transcript")
VECTOR_SIZE = int(os.getenv("QDRANT_VECTOR_SIZE", "384"))  # Default to 384, adjust as needed
try:
...
def retrieve_context(input_dict):
    try:
        ...
        context_texts = [h.payload.get("text", "") for h in hits]
        logger.info(f"Retrieved {len(context_texts)} context items")
        
        if not context_texts:
            logger.warning("No context found in Qdrant - using empty context")
        ...
        return {"question": input_dict["question"], "context": ["Error retrieving context from database."]}

def build_rag_prompt(input_dict):
    try:
        question = input_dict["question"]
        context = input_dict["context"]
        logger.info(f"Building RAG prompt for question: {question[:100]}...")
        ...
        return {"question": input_dict["question"], "prompt": f"Question: {input_dict['question']}\nAnswer:"}

def llm_mcp_call(input_dict):
    try:
        prompt = input_dict["prompt"]
        logger.info(f"Calling LLM with prompt: {prompt[:200]}...")
        ...
        if resp.status_code == 200:
            ...
            if assistant_content:
                logger.info(f"Generated LLM response: {assistant_content[:100]}...")
...
# Use pipe operator for sequence construction
...
async def ask(request: MCPRequest):
    try:
        ...
        logger.info(f"RAG pipeline completed, answer: {answer[:100]}...")
